...
### 10-29. 회피 여유(`inflation_radius`)와 로컬 회피 반응 거리 — 계산법과 근거

§10-26 으로 주행 자체가 되기 시작한 뒤, 실주행을 보니 **로봇이 벽에
스칠 정도로 붙어서 지나갔습니다.** 두 값이 모두 3~4 m 실험실 방 기준으로
잡혀 있던 게 원인이었습니다. 다음에 이 값을 다시 만질 때 재계산하지
않아도 되도록 근거를 남깁니다.

#### `inflation_radius` — 무엇으로부터의 거리인가

**장애물 셀에서 로봇 중심(base_link 원점)까지의 거리입니다.** 라이다
위치와는 무관하고, 로봇 몸체 바깥면 기준도 아닙니다. 로봇 반폭은 Nav2 가
footprint 로 **따로 겹쳐서** 판정합니다. 그래서:

```
실제 몸체 여유 = inflation_radius - (해당 방향 footprint 거리)
```

이 로봇 footprint 는 **앞뒤 비대칭**입니다
(`[[0.167,0.215],[0.167,-0.215],[-0.318,-0.215],[-0.318,0.215]]`):

| 방향 | 중심→외곽 | `0.22` 일 때 | `0.35` 일 때 |
|---|---|---|---|
| 좌우 | 0.215 m | **0.5 cm** | 13.5 cm |
| 앞 | 0.167 m | 5.3 cm | 18.3 cm |
| 뒤 | **0.318 m** | (음수, 여유 없음) | 3.2 cm |

**기존 0.22 는 측면 여유가 0.5 cm 였습니다** — 사실상 없는 것이고, 실제로
그렇게 보였습니다. 2026-08-25 에 **0.35** 로 올렸습니다(local/global 동일).

**뒤쪽(0.318 m)이 항상 가장 빡빡합니다.** 캐스터 쪽으로 몸체가 길어서인데,
현재 설정은 후진을 안 하므로(`min_vel_x: 0.0`) 뒤쪽 여유가 문제되는 것은
**제자리 회전할 때와 backup 복구 동작 때**뿐입니다. 뒤쪽까지 넉넉히
하려면 0.45~0.5 가 필요하지만 아래 대가가 커집니다.

**대가 — 좁은 통로.** 양쪽 벽에서 `inflation_radius` 만큼씩 고비용이 되므로
**폭이 `inflation_radius x 2` 미만인 통로는 planner 가 경로를 못 찾습니다.**
0.35 면 0.7 m, 0.5 면 1.0 m. 건물 복도는 보통 1 m 이상이라 0.35 는
문제없을 것으로 보지만 **좁은 구간에서 미검증**입니다. 경로를 못 찾는
구간이 나오면 이 값을 가장 먼저 의심하세요.

**local 과 global 은 반드시 같은 값으로.** 다르면 전역 경로가 "지나갈 수
있다"고 그린 곳을 로컬 컨트롤러가 막혔다고 보거나 그 반대가 됩니다.

#### 로컬 회피 반응 거리 — `local_costmap` 창 크기가 결정합니다

**장애물이 로컬 코스트맵에 찍히려면 두 조건을 모두 만족해야 합니다:**
1. 라이다 점이 `obstacle_max_range`(3.0 m) 이내
2. 그 점이 `local_costmap` 창 안 (rolling window, 로봇 따라다님)

**2x2 m 설정에서는 2번이 훨씬 빡빡해서 사실상 이게 한계였습니다** — 로봇
중심 ±1 m 밖은 로컬 코스트맵에 아예 안 들어갔습니다. DWB 는 로컬
코스트맵만 보고 궤적을 고르므로 **즉각 회피 반응 거리가 1 m 뿐**이었던
셈입니다(0.15 m/s 로 약 6.7초. 사람이 마주 걸어오면 훨씬 짧아집니다).

**로봇이 3 m 밖 장애물을 아예 모르는 것은 아닙니다** — global costmap 에도
같은 `/scan` 을 쓰는 obstacle_layer 가 있어 3 m 까지 마킹되고, 1Hz 로
전역 경로가 다시 그려집니다. 역할이 다릅니다:

| 코스트맵 | 감지 범위 | 역할 |
|---|---|---|
| local (창 크기가 상한) | 창 반경 | DWB **즉각 회피** — 실제로 궤적을 꺾음 |
| global (지도 전체) | `obstacle_max_range` 3.0 m | **경로 재계획** — 우회로를 다시 그림 |

2026-08-25 에 local 을 **6x6 m** 로 올렸습니다(사방 3 m, `obstacle_max_range`
와 일치). 계산 부담은 무시할 수준입니다 — 14,400 칸이고 전역은 이미 96만 칸.

**⚠ `width`/`height` 는 정수만 됩니다** (§10-19). 1.5 같은 실수를 넣으면
`InvalidParameterTypeException` 으로 controller_server 가 즉시 죽습니다.

#### 아직 검증 안 된 것

- 확대 후 **실제 회피 주행**(장애물을 우회해 `Recoveries: 0` 으로 도달) 미검증.
- **움직이는 사람** 회피 미검증 (§9 부터 계속 미해결).
- 좁은 통로에서 0.35 로 경로를 찾는지 미검증.
- `obstacle_max_range`(3.0)/`raytrace_max_range`(3.5)는 그대로 뒀습니다.
  이 값들은 §10-10 의 **철창 틈으로 빔이 새어 벽을 지우는 문제** 대책으로
  일부러 짧게 잡은 것입니다. 공항에는 철창이 없지만 **유리벽이 같은 문제를
  일으킬 수 있습니다**(빔 투과·이상 반사) — 현장에서 확인 후 조정할 것.

#### 실시간 인식은 확인됨

지도에 없던 물체(박스)를 로봇 앞 빈 공간에 놓자 로컬 코스트맵에 **새
장애물 셀이 즉시 생겼습니다**(2026-08-25 육안 확인). 즉 `obstacle_layer` 의
실시간 마킹은 동작합니다. 남은 것은 "인식한 것을 실제로 피해 가는가".

**RViz 에서 보는 법:** 원본 `Map`(흑백)이 아니라 `Global Planner` /
`Controller` 의 코스트맵을 봐야 합니다. lethal 은 짙은 남색, 그 주변
inflation 이 하늘색~분홍 그라데이션입니다. 화면 전체에 퍼진 색은 지도
(static_layer)에서 온 것이고, 실시간 라이다 기여분은 **로봇 주변 3 m
이내에만** 얹힙니다 — 통로 한가운데 흩어진 작은 덩어리들이 저상 라이다
로봇이 찍어준 낮은 장애물들로, static_layer 가 유지해주고 있는 것입니다
(§10-20 에서 의도한 동작).
