...
**바로 다음 작업은 실기 Nav2 입니다. 준비물이 아직 없습니다.**

`tetra_dsv_s_bringup` 에는 **Nav2 관련 파일이 하나도 없습니다.** 지금 있는
것은 `drive.launch.py`, `slam.launch.py` 와 config 4개(velodyne, imu, ekf,
slam_toolbox)뿐입니다. `ros2 launch ... navigation.launch.py` 같은 걸 바로
칠 수 없습니다. 만들어야 할 것:

- `config/nav2_params.yaml` — costmap(전역/지역), planner, controller,
  AMCL, behavior server. **시뮬 config 를 복사할 수 없습니다**:
  ① `use_sim_time: True` 로 되어 있고 ② 시뮬 패키지가 젯슨에
  sparse-checkout 제외라 파일 자체가 없습니다 (§1). 노트북/시뮬 쪽에서
  가져오거나 `nav2_bringup` 기본값에서 시작해야 합니다
- `launch/navigation.launch.py` — 구동부 + 센서 + EKF + map_server + AMCL
  + Nav2. `slam.launch.py` 에서 slam_toolbox 만 빼고 재사용하는 구조가 자연스럽습니다

**Nav2 를 켜기 전에 반드시 정할 것 두 가지 (둘 다 안전 문제):**

1. **철창 대책 (§10-10).** 지도를 손보지 않으면 planner 가 철창 틈으로 경로를
   냅니다. PGM 편집이 가장 간단합니다. 안 하고 돌리면 로봇이 철창으로 갑니다.
2. **1 m 아래 사각지대 (§10-9).** 책상·의자·박스가 지도에 없습니다. 자율주행
   중 로봇이 그리로 들어갑니다. 최소한 **첫 주행은 사람이 붙어서 비상정지에
   손을 얹고** 해야 합니다. 근본 대책은 RealSense D455 인데 드라이버 미설치.

**그 밖에:**

- ~~방위 4도 오차의 정체~~ — **해결됨. 출발점 복귀로 loop closure 를 거세요**
  (§10-15). 매핑 마무리 절차로 굳히는 게 좋습니다.
- **지도 경계 닫기 (§10-17, §10-10).** 메인 지도의 자유 공간 15.6 m^2 중
  절반이 아직 철창 누수입니다 (실제 방 7.2 m^2). 방 경계를 검은 선으로 닫는
  편집이 남아 있습니다. **닫기 전에는 Nav2 를 켜지 마세요.**
  선을 그은 뒤 "방 안에서 도달 못 하는 자유 공간을 미지로 되돌리는" 정리는
  자동화 가능합니다 (연결영역 채우기).
